...
from src.risk.metrics import _lane_id, _safe_position, _safe_speed, collect_step_metrics
...
def _risk_step(env, risk_params: Dict) -> Dict[str, float]:
    ...
    step = {
        "ttc_min": float(metrics.get("ttc_min", 1e6)),
        "thw_min": float(metrics.get("thw_min", 1e6)),
        "drac_max": float(metrics.get("drac_max", 0.0)),
        "density": float(metrics.get("density", 0.0)),
        "risk_score": float(risk_score),
        "target_gap_size": float(gap.get("gap_size", 0.0)),
        "target_rear_distance": float(rear_distance),
        "target_rear_relative_speed": float(gap.get("rear_relative_speed", 0.0)),
...
def evaluate_model(
    ...
    risk_rows: List[Dict] = []
    frames = []
    thresholds = risk_params.get("thresholds", {})
    for episode_idx in range(int(episodes)):
        ...
        while not (terminated or truncated):
            ...
            if prev_lane is not None and lane_now is not None and lane_now != prev_lane:
                lane_change_count += 1
            prev_lane = lane_now
            ttc_min = min(ttc_min, step_risk["ttc_min"])
            thw_min = min(thw_min, step_risk["thw_min"])
            drac_max = max(drac_max, step_risk["drac_max"])
            risk_exposure += step_risk["risk_score"]
            risk_score_max = max(risk_score_max, step_risk["risk_score"])
            if (
                step_risk["ttc_min"] < float(thresholds.get("ttc_near_miss", 2.0))
                or step_risk["thw_min"] < float(thresholds.get("thw_near_miss", 1.0))
                or step_risk["drac_max"] > float(thresholds.get("drac_near_miss", 3.0))
            ):
                near_miss_count += 1
            target_gap_values.append(step_risk["target_gap_size"])
            target_rear_distance_values.append(step_risk["target_rear_distance"])
            target_rear_relative_speed_values.append(step_risk["target_rear_relative_speed"])
            p_yield_values.append(step_risk["p_yield"])
            p_block_values.append(step_risk["p_block"])
            if episode_idx == 0:
                trace_rows.append({
                    "method": method,
                    "seed": int(seed),
...
def aggregate_eval_metrics(csv_paths: Iterable[Path]) -> pd.DataFrame:
    ...
    summary = df.groupby("method", as_index=False).agg(
        ...
        lane_change_count_mean=("lane_change_count", "mean"),
        lane_change_count_std=("lane_change_count", "std"),
    )
